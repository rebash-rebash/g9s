...
func renderVMDetails(vm model.VM, disks []model.Disk, monitoringAvailable, metricsLoading bool, utilization model.Utilization, ioStats model.IOStats, metricsErr error, catalog cost.Catalog) string {
	...
	if vm.Status != "RUNNING" { body = append(body, panelStyle.Width(94).Render(lipgloss.JoinVertical(lipgloss.Left, metricsTitle, "", mutedStyle.Render("VM is not running — live utilization is not evaluated.")))) } else if !monitoringAvailable { body = append(body, panelStyle.Width(94).Render(lipgloss.JoinVertical(lipgloss.Left, metricsTitle, "", mutedStyle.Render("Cloud Monitoring is unavailable.")))) } else if metricsLoading { body = append(body, panelStyle.Width(94).Render(lipgloss.JoinVertical(lipgloss.Left, metricsTitle, "", mutedStyle.Render("Loading Cloud Monitoring metrics...")))) } else if metricsErr != nil { body = append(body, panelStyle.Width(94).Render(lipgloss.JoinVertical(lipgloss.Left, metricsTitle, "", statusBadStyle.Render("Monitoring error: "+metricsErr.Error())))) } else if utilization.Average == nil { body = append(body, panelStyle.Width(94).Render(lipgloss.JoinVertical(lipgloss.Left, metricsTitle, "", mutedStyle.Render("No CPU metric data available.")))) } else {
